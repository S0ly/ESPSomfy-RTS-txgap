...
struct transceiver_config_t {
    ...
    uint16_t txGap = 3000;            // Min ms between RF commands to DIFFERENT shades. Serializes bursts so far motors don't drop frames when many shades are commanded at once. Same-shade frames (a blind's own up/down/my sequence) are NEVER delayed. 0 = disabled.
/*    
